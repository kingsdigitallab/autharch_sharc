...
class RecordSearch(FacetedSearch):
    ...
    def search(self):
        ...
        if self._category:
            s = s.query("prefix", category__lowercase=self._category)
        if self._reference:
            try:
                s = s.filter("prefix", reference=self._reference)
            except TypeError:
                pass
        if self._unittitle:
            s = s.query("prefix", unittitle=self._unittitle)
        return s
